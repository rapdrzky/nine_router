...
![Version](https://img.shields.io/badge/9router-v0.5.35-green)
...
- Image version is pinned (`v0.5.35`), so builds are reproducible.
- Health check on `/dashboard` with a 120-second timeout.
...
9Router sits between your coding tools and AI providers:
...
When a subscription quota runs out, requests fall back to cheap providers, then to free ones. The result is no mid-coding interruptions and lower cost.
...
9Router is MIT licensed — see the [upstream repository](https://github.com/decolua/9router). This deployment wrapper follows the same license.
